...
if __name__ == "__main__":
    ...
    size_min, size_max = 2, 50           # maze sizes from 2x2 up to 50x50
    runs_per_size = 1000                 # number of trials per size
    start = (0, 0)                       # fixed start node
    memory_fraction = 0.5                # memory bound fraction of total nodes
    ...
    print("\n===== Memory-Bounded A* Simulation Results =====")
    # Display interactive table
print(df.to_string(index=False))  # fallback console print
...
plt.title(f"Memory-Bounded A* Metrics vs Maze Size ({runs_per_size} trials)")
...
plt.title(f"Memory-Bounded A* Avg Execution Time vs Maze Size ({runs_per_size} trials)")
plt.xlabel('Maze Size (N x N)')
